...
def cost_impact(country, objective, objective_progs, eff_progs, cov=0.95, type='LBE'):
    progval = {'Balanced energy-protein supplementation': cov, 'Calcium supplementation': cov,
               ...
               'Multiple micronutrient supplementation': cov, 'IPTp': cov,
...
def run_global_bounds(date, type, prog_order):
    ...
    stunt_progs = prog_order['stunting']['Program']
    wast_progs = prog_order['wasting']['Program']
    anaem_progs = prog_order['anaemia']['Program']
    mort_progs = prog_order['mortality']['Program']
    ...
    for o, outcome in enumerate(['stunting', 'wasting', 'anaemia', 'mortality']):
        ...
        for round in list(range(num_CE[o])):
            ...
            total_impact = {}
            total_CE = {}
            impact_cost = partial(cost_impact, objective=outcome, objective_progs=progval, eff_progs=effective_progs, type=type)
    ...
    print(prog_choices)
    CE_book = xw.Workbook('Results/' + date + '/' + type + '_Global_cost_effectiveness' + date +'.xlsx')
