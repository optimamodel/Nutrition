...
export default {
  ...
  methods: {
    ...
    updateProjectSummaries(setActiveID) {
      ...
      sciris.start(this)
      sciris.rpc('jsonify_projects', [this.$store.state.currentUser.username]) // Get the current user's project summaries from the server.
        .then(response => {
    ...
    addDemoProject() {
      ...
      sciris.start(this);
      sciris.rpc('add_demo_project', [this.$store.state.currentUser.username]) // Have the server create a new project.
        .then(response => {
    ...
    createNewProject() {
      ...
      sciris.start(this) // Start indicating progress.
      var username = this.$store.state.currentUser.username
      sciris.download('create_new_project', [username, this.proj_name]) // Have the server create a new project.
        .then(response => {
          this.updateProjectSummaries(null); // Update the project summaries so the new project shows up on the list.
          sciris.succeed(this, 'New project "' + this.proj_name + '" created') // Indicate success.
        })
        .catch(error => {
    ...
    uploadProjectFromFile() {
      ...
      sciris.upload('upload_project', [this.$store.state.currentUser.username], {}, '.prj') // Have the server upload the project.
        .then(response => {
          sciris.start(this)  // This line needs to be here to avoid the spinner being up during the user modal.
          this.updateProjectSummaries(response.data.projectID) // Update the project summaries so the new project shows up on the list.
    ...
    copyProject(uid) {
      ...
      sciris.rpc('copy_project', [uid]) // Have the server copy the project, giving it a new name.
        .then(response => {
          ...
          sciris.succeed(this, 'Project "'+matchProject.project.name+'" copied')    // Indicate success.
        })
        .catch(error => {
          sciris.fail(this, 'Could not copy project', error) // Indicate failure.
        })
    },
    ...
    renameProject(projectSummary) {
      ...
      if (projectSummary.renaming === '') { // If the project is not in a mode to be renamed, make it so.
        ...
        sciris.start(this)
        sciris.rpc('rename_project', [newProjectSummary]) // Have the server change the name of the project by passing in the new copy of the summary.
          .then(response => {
          ...
          .catch(error => {
            sciris.fail(this, 'Could not rename project', error) // Indicate failure.
          })
      }
    ...
    downloadProjectFile(uid) {
      ...
      sciris.start(this) // Start indicating progress.
      sciris.download('download_project', [uid]) // Make the server call to download the project to a .prj file.
    ...
    downloadDefaults(uid) {  // ATOMICA-NUTRITION DIFFERENCE
      ...
      sciris.start(this)
      sciris.download('download_defaults', [uid])
      .then(response => {
        sciris.succeed(this, '')  // No green popup message.        
      })
      .catch(error => {
        sciris.fail(this, 'Could not download defaults', error)     
      })      
    },
    ...
    uploadDatabook(uid) {
      ...
      sciris.upload('upload_databook', [uid], {}, '.xlsx')
        .then(response => {
          ...
          sciris.succeed(this, 'Data uploaded to project') // Indicate success.
        })
        .catch(error => {
          sciris.fail(this, 'Could not upload data', error) // Indicate failure.
        })
    },

    uploadDefaults(uid) {
      console.log('uploadDefaults() called')     
      sciris.upload('upload_defaults', [uid], {}, '.xlsx')
        .then(response => {
          sciris.start(this, 'Uploading defaults...')
          this.updateProjectSummaries(uid) // Update the project summaries
          sciris.succeed(this, 'Defaults uploaded to project') // Indicate success.
        })
        .catch(error => {
          sciris.fail(this, 'Could not upload defaults', error) // Indicate failure.
        })
    },
    ...
    deleteSelectedProjects() {
      ...
      if (selectProjectsUIDs.length > 0) { // Have the server delete the selected projects.
        sciris.start(this)
        sciris.rpc('delete_projects', [selectProjectsUIDs, this.$store.state.currentUser.username])
          .then(response => {
    ...
    downloadSelectedProjects() {
      ...
      if (selectProjectsUIDs.length > 0) { // Have the server download the selected projects.
        sciris.start(this)
        sciris.download('download_projects', [selectProjectsUIDs, this.$store.state.currentUser.username])
          .then(response => {
            // Indicate success.
            sciris.succeed(this, '')  // No green popup message.
          })
          .catch(error => {
            sciris.fail(this, 'Could not download project(s)', error) // Indicate failure.
          })
      }
    }
  }
