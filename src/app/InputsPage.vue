...
export default {
  ...
  methods: {
    ...
    getSheetData() {
      ...
      sciris.start(this, 'Getting data...')
      sciris.rpc('get_sheet_data', [this.projectID]) // Make the server call to download the framework to a .prj file.
        .then(response => {
          ...
          sciris.succeed(this, 'Data loaded')
        })
        .catch(error => {
          sciris.fail(this, 'Could not get sheet data', error)
        })
    },

    saveSheetData() {
      console.log('saveSheetData() called')
      sciris.start(this, 'Saving changes...')
      sciris.rpc('save_sheet_data', [this.projectID, this.sheetTables]) // Make the server call to download the framework to a .prj file.
        .then(response => {
          sciris.succeed(this, 'Data saved')
        })
        .catch(error => {
          sciris.fail(this, 'Could not save sheet data', error)
        })
    },
    ...
    downloadDatabook() {
      ...
      sciris.download('download_databook', [this.projectID])
        .then(response => {
    ...
    uploadDatabook() {
      ...
      sciris.upload('upload_databook', [this.projectID], {}, '.xlsx')
        .then(response => {
        ...
        .catch(error => {
          sciris.fail(this, 'Could not upload data', error)
        })
    },
    ...
    renameDataset() {
      ...
      sciris.start(this)
      sciris.rpc('rename_dataset', [this.projectID, this.origDatasetName, this.activeDataset]) // Have the server copy the project, giving it a new name.
        .then(response => {
          ...
          sciris.succeed(this, 'Dataset "'+this.activeDataset+'" renamed') // Indicate success.
        })
        .catch(error => {
          sciris.fail(this, 'Could not rename dataset', error)
        })
    },
    ...
    copyDataset() {
      ...
      sciris.start(this)
      sciris.rpc('copy_dataset', [this.projectID, this.activeDataset]) // Have the server copy the project, giving it a new name.
        .then(response => {
          this.updateDatasets() // Update the project summaries so the copied program shows up on the list.
          // TODO: look into whether the above line is necessary
          this.activeDataset = response.data
          sciris.succeed(this, 'Dataset "'+this.activeDataset+'" copied') // Indicate success.
        })
        .catch(error => {
          sciris.fail(this, 'Could not copy dataset', error)
        })
    },
    ...
    deleteDataset() {
      ...
      sciris.start(this)
      sciris.rpc('delete_dataset', [this.projectID, this.activeDataset]) // Have the server copy the project, giving it a new name.
        .then(response => {
          this.updateDatasets() // Update the project summaries so the copied program shows up on the list.
          // TODO: look into whether the above line is necessary
          sciris.succeed(this, 'Dataset "'+this.activeDataset+'" deleted') // Indicate success.
        })
        .catch(error => {
          sciris.fail(this, 'Cannot delete last dataset: ensure there are at least 2 datasets before deleting one', error)
        })
    },
