...
export default {
  ...
  methods: {
    ...
    addScenModal(scen_type) {
      ...
      sciris.rpc('get_default_scen', [this.projectID, scen_type])
        .then(response => {
    ...
    convertScen(scenSummary) {
      ...
      sciris.start(this)
      sciris.rpc('convert_scen', [this.projectID, scenSummary.name])
        .then( response => {
          sciris.succeed(this, 'Scenario converted')
          this.getScenSummaries()
        })
        .catch(error => {
          sciris.fail(this, 'Could not convert scenario', error)
        })
    },
    ...
    runScens() {
      ...
      sciris.rpc('set_scen_info', [this.projectID, this.scenSummaries]) // Make sure they're saved first
        .then(response => {
          sciris.rpc('run_scens', [this.projectID]) // Go to the server to get the results
            .then(response => {
            ...
            .catch(error => {
              ...
              sciris.fail(this, 'Could not run scenarios', error) // Indicate failure.
            })
        })
        .catch(error => {
