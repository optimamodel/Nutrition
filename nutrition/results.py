...
class ScenResult(sc.prettyobj):
    ...
    def get_childscens(self):
        ...
        for prog in progset:
            ...
            kwargs = {"name": prog, "model_name": self.model_name, "scen_type": "budget", "progvals": new_progvals, "enforce_constraints_year": 0}
            allkwargs.append(kwargs)
        return allkwargs
...
def write_results(results, reduced_results={}, projname=None, filename=None, folder=None, full_outcomes=False, full_rows=True):
    ...
    headers = [[_("Scenario"), _("Estimate"), _("Outcome")] + years + [_("Total (discounted)")]]
    for r, res in enumerate(results):
        if res.name != _("Excess budget") and resampled_key_str not in res.name:
            econo_outcomes = []
            for out in outcomes:
