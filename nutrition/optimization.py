...
class Optim(sc.prettyobj):
    ...
    def get_kwargs(self, model, weights, mult, keep_inds):
        ...
        kwargs = {"model": model, "free": (free+sum(fixed)) * mult - sum(fixed) if mult > 1 else free * mult, "fixed": fixed, "weights": weights, "keep_inds": keep_inds}
        # if free == 0:
    ...
    ######### OPTIMIZATION ##########
    @translate
    def run_optim(self, model, maxiter=1, swarmsize=None, maxtime=600, parallel=True, num_procs=None, runbalanced=False, base=None):
        if parallel:
            how = "parallel"
...
def make_default_optim(modelname=None, basename="Maximize thrive", locale=None):
    ...
    kwargs1 = {
        ...
        "prog_set": [
            _("Calcium supplementation"),
            _("Cash transfers"),
            _("Delayed cord clamping"),
            _("IFA fortification of wheat flour"),
            _("IPTp"),
            ...
            _("Small quantity lipid-based nutrition supplements"),
            _("Treatment of SAM"),
            _("Vitamin A supplementation"),
            _("Zinc for treatment + ORS"),
            _("Zinc supplementation"),
        ],
        "fix_curr": False,
        "add_funds": 0,
        "filter_progs": True,
