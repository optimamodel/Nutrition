def asd(function, x, args=None, stepsize=0.1, sinc=2, sdec=2, pinc=2, pdec=2,
    ...
    x, origshape = consistentshape(x,
                                   origshape=True)  # Turn it into a vector but keep the original shape (not necessarily class, though)
    nparams = len(x)  # Number of parameters

    # Set initial parameter selection probabilities -- uniform by default
    if pinitial is None:
        probabilities = ones(2 * nparams)
    else:
    ...
    if any(stepsizes == 0): stepsizes[stepsizes == 0] = mean(
        stepsizes[stepsizes != 0])  # Replace step sizes of zeros with the mean of non-zero entries
    ...
    while True:
        ...
        if verbose == 1: print(offset + label + 'Iteration %i; elapsed %0.1f s; objective: %0.3e' % (
        count, time() - start, fval))  # For more verbose, use other print statement below
        ...
        if not inrange:
            if verbose >= 2: print(
                        '======== Can\'t find parameters within range after %i tries, terminating ========' % maxrangeiters)
            break
        ...
        relerrorhistory[mod(count, stalliters)] = max(0, fval / float(
            fvalnew) - 1.0)  # Keep track of improvements in the error
        ...
        if fvalnew < fvalold:  # New parameter set is better than previous one
            ...
            break
        if verbose == 2: print(offset + label + ' step %i (%0.1f s) %s (orig: %s | best:%s | new:%s | diff:%s)' % (
                    (count, time() - start, flag) + multisigfig([fvalorig, fvalold, fvalnew, fvalnew - fvalold])))
        if verbose >= 3: print(
                    label + '\nstep = %i\nelapsed = %s\nfval = %s\nchoice = %s\npar = %s\npm = %s\norigval = %s\nnewval = %s\ninrange = %s' % (
            count, time() - start, fval, choice, par, pm, xold[par], xnew[par], inrange))
    ...
    if verbose >= 2: print('=== %s %s (%i steps, orig: %s | best: %s | ratio: %s) ===' % (
                (label, exitreason, count) + multisigfig([fvals[0], fvals[-1], fvals[-1] / fvals[0]])))
    if fulloutput:
        details = dict()
    ...
    for i in range(n):
        ...
        try:
            if x == 0:
                ...
                digits = int(abs(magnitude) + max(0, sigfigs - max(0, magnitude) - 1) + 1 + (x < 0) + (
                            abs(x) < 1))  # one because, one for decimal, one for minus
