...
def plot_outputs_reduced(all_res, all_reduce, seq, name, locale=None):
    ...
    for i, outcome in enumerate(outcomes):
        ...
        if "pop" in outcome or ("mam" in outcome and seq) or ("sga" in outcome and seq) or ("months" in outcome and not seq) or ("years" in outcome and not seq) or ("num_pw" in outcome and not seq):
            continue
        else:
            fig = pl.figure(figsize=fig_size)
            ax = fig.add_axes(ax_size)
