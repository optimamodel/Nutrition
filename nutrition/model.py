...
class Model(sc.prettyobj):
    ...
    def _track_prevs(self):
        ...
        if self.children.frac_risk('an') < 0.5 and self.children.frac_risk('an') > 0.05:
        ...
        if self.pw.frac_risk('an') < 0.09 and self.pw.frac_risk('an') > 0.01:
            ...
            self.pw_anaemprev_thresh[self.year] = 5e5 * (self.pw.frac_risk('an') - 0.09)
    ...
    def _wasting_trans(self, age_group):
        ...
        eff = age_group.prog_eff[('Management of MAM', 'SAM', 'Effectiveness incidence')]
        ...
        age_group.fromSAMtoMAMupdate['SAM'] = 1 - eff * (1. - age_group.wastingTreatmentUpdate['MAM'])
