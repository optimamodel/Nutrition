...
def cost_impact(country, objective, objective_progs, eff_progs):
    ...
    p.load_data(country='Efficacy upper bounds/ube_costed_baseline_' + country, name=country, time_trend=False)
    effective_progs = {}
    if eff_progs:
        if len(eff_progs.keys()) > 1:
            for prog in eff_progs.keys():
...
progval = {'Balanced energy-protein supplementation': cov, 'Calcium supplementation': cov,
     ...
     'IFAS for pregnant women (community)': cov, 'Iron and iodine fortification of salt': cov,
     'IYCF 1': cov, 'Kangaroo mother care': cov, 'Lipid-based nutrition supplements': cov,
     'Multiple micronutrient supplementation': cov, 'IPTp': cov, 'Long-lasting insecticide-treated bednets': cov,
     'Zinc for treatment + ORS': cov, 'Public provision of complementary foods': cov, 'Treatment of SAM': cov,
     'Vitamin A supplementation': cov, 'Cash transfers': cov}

prog_list = sc.dcp(progval.keys())

stunt_progs = ['IPTp', 'IYCF 1', 'Vitamin A supplementation', 'Public provision of complementary foods', 
               'Multiple micronutrient supplementation', 'Long-lasting insecticide-treated bednets',
               'Balanced energy-protein supplementation', 'Lipid-based nutrition supplements']
wast_progs = ['Vitamin A supplementation', 'Cash transfers']
anaem_progs = ['IPTp', 'IFAS for pregnant women (community)', 'Iron and iodine fortification of salt', 'IFAS (community)',
               'Long-lasting insecticide-treated bednets', 'IFA fortification of maize', 'Multiple micronutrient supplementation']
mort_progs = ['IPTp', 'Kangaroo mother care', 'Zinc for treatment + ORS', 'IYCF 1', 'Vitamin A supplementation',
              'Multiple micronutrient supplementation', 'Long-lasting insecticide-treated bednets', 'IFA fortification of maize',
              'Balanced energy-protein supplementation', 'Public provision of complementary foods', 'Cash transfers',
              'Treatment of SAM', 'Lipid-based nutrition supplements']
...
for o, outcome in enumerate(['stunting', 'wasting', 'anaemia', 'mortality']):
    ...
    for round in list(range(num_CE[o])):
        ...
        if total_CE[program_choice] <= 0 or total_CE[program_choice] == np.nan or total_CE[program_choice] == np.inf:
            prog_choices[outcome]['Cost effectiveness'].append(0)
	    prog_choices[outcome]['Impact'].append(0)
...
print(prog_choices)
CE_book = xw.Workbook('UBE_Global_cost_effectiveness04032020.xlsx')
