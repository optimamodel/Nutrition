...
def make_kwargs_opt(name, progset, weights, mults, modelname='Demo', fixcurr=False, addfunds=0.):
    kwargs = {'name': name,
               'model_name': modelname,
               'weights': weights,
               'mults': mults,
...
d = make_kwargs('2.2d', {'Public provision of complementary foods': cov,
...
for age in ss.child_ages:
    mydata.prog_data.prog_target['IYCF 1'][age] = .37
...
a = make_kwargs('3.2a', {'Cash transfers': cov,
...
cc = make_kwargs('4.2c', {'IFAS for pregnant women (community)': cov,
                        'Multiple micronutrient supplementation': cov})
dd = make_kwargs('4.2d', {'IPTp': cov,
                        'Multiple micronutrient supplementation': cov})
ee = make_kwargs('4.2e', {'IPTp': [0],
                        'Multiple micronutrient supplementation': cov})
